...
REMR3DECL(int) REMR3EmulateInstruction(PVM pVM)
{
    ...
    if (VBOX_SUCCESS(rc))
    {
        ...
        switch (rc)
        {
            ...
            /*
             * An EM RC was raised (VMR3Reset/Suspend/PowerOff).
             */
            case EXCP_RC:
...
/**
 * Set page table/dir entry. (called from tlb_set_page)
 *
 * @param   env         Pointer to cpu environment.
 */
void remR3SetPage(CPUState *env, CPUTLBEntry *pRead,  CPUTLBEntry *pWrite, int prot, int is_user)
{
     ...
     * Update the control registers before calling PGMFlushPage.
     */
    PCPUMCTX pCtx = (PCPUMCTX)env->pVM->rem.s.pCtx;
    pCtx->cr0 = env->cr[0];
    pCtx->cr3 = env->cr[3];
    pCtx->cr4 = env->cr[4];
     ...
     * Let PGM do the rest.
     */
    int rc = PGMInvalidatePage(env->pVM, (RTGCPTR)virt_addr);
    if (VBOX_FAILURE(rc))
    {
...
{
    LogFlow(("REMR3NotifyPhysReserve: GCPhys=%VGp cb=%d\n", GCPhys, cb));
    VM_ASSERT_EMT(pVM);
...
{
    LogFlow(("REMR3NotifyHandlerPhysicalRegister: enmType=%d GCPhys=%VGp cb=%d fHasHCHandler=%d\n",
...
{
    LogFlow(("REMR3NotifyHandlerPhysicalDeregister: enmType=%d GCPhys=%VGp cb=%d fHasHCHandler=%d pvHCPtr=%p RAM=%08x\n",
...
{
    LogFlow(("REMR3NotifyHandlerPhysicalModify: enmType=%d GCPhysOld=%VGp GCPhysNew=%VGp cb=%d fHasHCHandler=%d pvHCPtr=%p\n",
