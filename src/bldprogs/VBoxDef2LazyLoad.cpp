...
static RTEXITCODE generateOutputInnerArm64(FILE *pOutput)
{
    ...
    for (PMYEXPORT pExp = g_pExpHead; pExp; pExp = pExp->pNext)
    {
        if (!pExp->fNoName)
            fprintf(pOutput,
                    ...
                    "#else\n"
                    "    ldr     x9, [x9, PAGEOFF(g_sz%s)]\n"
                    "#endif\n"
                    "    adrp    x10, PAGE(NAME(%s%s))\n"
                    "#ifndef ASM_FORMAT_ELF\n"
                    "    add     x10, x10, PAGEOFF(NAME(%s%s))\n"
                    "#else\n"
                    "    ldr     x10, [x10, PAGEOFF(NAME(%s%s))]\n"
                    "#endif\n"
                    "    mov     x16, x30\n"
                    "    bl      LazyLoadResolver\n"
                    "    mov     x30, x16\n"
    ...
    fprintf(pOutput,
            ...
            "#else\n"
            "    ldr     x0, [x0, PAGEOFF(g_szFailLoadFmt)]\n"
            "#endif\n"
            "    stp     x1, x2, [sp]\n"
            "    str     x3,     [sp, #16]\n"
    ...
    if (!g_fSystemLibrary)
        fprintf(pOutput,
                ...
                "#else\n"
                "    ldr     x1, [x1, PAGEOFF(g_hMod)]\n"
                "#endif\n"
                "    adrp    x0, PAGE(g_szLibrary)\n"
                "#ifndef ASM_FORMAT_ELF\n"
                "    add     x0, x0, PAGEOFF(g_szLibrary)\n"
                "#else\n"
                "    ldr     x0, [x0, PAGEOFF(g_szLibrary)]\n"
                "#endif\n"
    ...
    if (g_fWithExplictLoadFunction)
    {
        ...
        fprintf(pOutput,
                ...
                "#else\n"
                "    ldr      x22, [x22, PAGEOFF(g_szzNames)]\n"
                "#endif\n"
                "    adrp     x23, PAGE(g_apfnImports)\n"
                "#ifndef ASM_FORMAT_ELF\n"
                "    add      x23, x23, PAGEOFF(g_apfnImports)\n"
                "#else\n"
                "    ldr      x23, [x23, PAGEOFF(g_apfnImports)]\n"
                "#endif\n"
