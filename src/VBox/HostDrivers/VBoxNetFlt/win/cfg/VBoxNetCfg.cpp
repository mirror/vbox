...
static HRESULT vboxNetCfgWinInfQueryKeyValue(HINF hInf, LPCWSTR lpszSection, LPCWSTR lpszKey, DWORD iValue, LPWSTR *lppszValue)
{
    INFCONTEXT InfContext;
    DWORD winEr;

    if (!SetupFindFirstLineW(hInf, lpszSection, lpszKey, &InfContext))
    {
        winEr = GetLastError();
        Log(__FUNCTION__ ": SetupFindFirstLine failed WinEr (%d) for Section(%S), Key(%S)\n", winEr, lpszSection, lpszKey);
        return HRESULT_FROM_WIN32(winEr);
    }

    DWORD cbValue;
    if (SetupGetStringFieldW(&InfContext, iValue, NULL, 0, &cbValue))
    {
        Assert(0);
        /* SetupGetStringField succeeded when it should not */
        DbgLog(__FUNCTION__ ": SetupGetStringField succeeded when it should not, for Section(%S), Key(%S), iValue(%d)\n", lpszSection, lpszKey, iValue);
        return E_FAIL;
    }
    ...
    {
        Log(__FUNCTION__ ": SetupGetStringField failed WinEr (%d) for Section(%S), Key(%S), iValue(%d)\n", winEr, lpszSection, lpszKey, iValue);
        return HRESULT_FROM_WIN32(winEr);
    }

    LPWSTR lpszValue = (LPWSTR)CoTaskMemAlloc(cbValue * sizeof (lpszValue[0]));
    Assert(lpszValue);
    if (!lpszValue)
    {
        Log(__FUNCTION__ ": SetCoTaskMemAlloc failed to alloc mem of size (%d), for Section(%S), Key(%S), iValue(%d)\n", cbValue * sizeof (lpszValue[0]), winEr, lpszSection, lpszKey, iValue);
        return E_FAIL;
    }

    if (!SetupGetStringFieldW(&InfContext, iValue, lpszValue, cbValue, &cbValue))
    {
        winEr = GetLastError();
        Log(__FUNCTION__ ": SetupGetStringField failed WinEr (%d) for Section(%S), Key(%S), , iValue(%d)\n", winEr, lpszSection, lpszKey, iValue);
        Assert(0);
...
static HRESULT vboxNetCfgWinInfQueryFirstPnPId(HINF hInf, LPWSTR *lppszPnPId)
{
    LPWSTR lpszModels;
    LPWSTR lpszPnPId;
    HRESULT hr = vboxNetCfgWinInfQueryKeyValue(hInf, L"Manufacturer", NULL, 1, &lpszModels);
    if (hr != S_OK)
    {
        DbgLog(__FUNCTION__ ": vboxNetCfgWinRegQueryKeyValue for Manufacturer failed, hr = (0x%x)\n", hr);
        return hr;
    }

    hr = vboxNetCfgWinInfQueryKeyValue(hInf, lpszModels, NULL, 2, &lpszPnPId);
    /* free models string right away */
    CoTaskMemFree(lpszModels);
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": vboxNetCfgWinRegQueryKeyValue for models failed, hr = (0x%x)\n", hr);
        return hr;
    }
...
static bool vboxNetCfgWinInfEnumerationCallback(LPCWSTR lpszFileName, PVOID pCtxt);

static HRESULT vboxNetCfgWinInfCopyEx(IN LPCWSTR lpszInfPath, IN DWORD fCopyStyle, OUT LPWSTR lpszDstName, IN DWORD cbDstName, OUT PDWORD pcbDstNameSize, OUT LPWSTR* lpszDstNameComponent)
{
            ...
            lpszDstNameComponent))
    {
        ...
        Log(__FUNCTION__ ": SetupCopyOEMInf fail winEr (%d)\n", winEr);
        return HRESULT_FROM_WIN32(winEr);
    }
...
static HRESULT vboxNetCfgWinInfCopy(IN LPCWSTR lpszInfPath)
{
    return vboxNetCfgWinInfCopyEx(lpszInfPath, 0, NULL, 0, NULL, NULL);
}

VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinInfInstall(IN LPCWSTR lpszInfPath)
{
    return vboxNetCfgWinInfCopy(lpszInfPath);
}

VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinInfUninstall(IN LPCWSTR lpszInfPath, DWORD fFlags)
{
    if (!SetupUninstallOEMInfW(lpszInfPath, fFlags, NULL /*__in PVOID Reserved == NULL */))
    {
        DWORD winEr = GetLastError();
        Log(__FUNCTION__ ": SetupUninstallOEMInf failed for file (%S), winEr (%d)\n", lpszInfPath, winEr);
        Assert(0);
        return HRESULT_FROM_WIN32(winEr);
    }
    return S_OK;
                    ...
                    ))
        {
            ...
            for (DWORD i = 0; ; i++)
            {
                        ...
                        ))
                {
                    ...
                    {
                        DWORD winEr = GetLastError();
                        Log(__FUNCTION__": SetupDiGetDriverInfoDetail fail winEr (%d), size(%d)", winEr, dwReq);
//                        Assert(0);
                    }
...
VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinInfUninstallAll(IN const GUID * pGuidClass, IN LPCWSTR lpszClassName, IN LPCWSTR lpszPnPId, IN DWORD Flags)
{
    ...
    HRESULT hr = vboxNetCfgWinCollectInfs(pGuidClass, lpszPnPId, list);
    if (hr == S_OK)
    {
...
static HRESULT vboxNetCfgWinEnumFiles(LPCWSTR pPattern, PFNVBOXNETCFG_ENUMERATION_CALLBACK pfnCallback, PVOID pContext)
{
    ...
    if (hEnum != INVALID_HANDLE_VALUE)
    {
        ...
        do
        {
            ...
            if (!bNext)
            {
                ...
                {
                    Log(__FUNCTION__": FindNextFile fail winEr (%d)\n", winEr);
                    Assert(0);
    ...
    else
    {
        ...
        if (winEr != ERROR_NO_MORE_FILES)
        {
            Log(__FUNCTION__": FindFirstFile fail winEr (%d)\n", winEr);
            Assert(0);
            hr = HRESULT_FROM_WIN32(winEr);
        }
    }
...
static bool vboxNetCfgWinInfEnumerationCallback(LPCWSTR lpszFileName, PVOID pCtxt)
{
    ...
    DWORD winEr;
//    Log("vboxNetCfgWinInfEnumerationCallback: pFileName (%S)\n", pFileName);
    ...
    if (hInf == INVALID_HANDLE_VALUE)
    {
        ...
        {
            DbgLog(__FUNCTION__ ": SetupOpenInfFileW err winEr (%d)\n", winEr);
        }
    ...
    LPWSTR lpszPnPId;
    HRESULT hr = vboxNetCfgWinInfQueryFirstPnPId(hInf, &lpszPnPId);
    if (hr == S_OK)
    {
    ...
    {
        DbgLog(__FUNCTION__ ": vboxNetCfgWinInfQueryFirstPnPId failed, hr = (0x%x)\n", hr);
    }
...
static HRESULT VBoxNetCfgWinInfUninstallAll(LPCWSTR lpszClassName, LPCWSTR lpszPnPId, DWORD Flags)
{
    ...
    if (hr == S_OK)
    {
        ...
        {
            Log(__FUNCTION__": vboxNetCfgWinEnumFiles failed, hr = (0x%x)\n", hr);
        }
    }
    else
    {
        Log(__FUNCTION__": SHGetFolderPathW failed, hr = (0x%x)\n", hr);
    }
                          ...
                          OUT LPWSTR *ppszwClientDescription)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": QueryInterface failed, hr (0x%x)\n", hr);
        return hr;
    }
    ...
    if (hr == S_FALSE)
    {
        Log(__FUNCTION__ ": write lock busy\n");
    }
    else if (hr != S_OK)
    {
        Log(__FUNCTION__ ": AcquireWriteLock failed hr (0x%x)\n", hr);
    }
...
static HRESULT vboxNetCfgWinINetCfgUnlock(IN INetCfg *pNetCfg)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": ReleaseWriteLock failed hr (0x%x)\n", hr);
    }
                          ...
                          OUT LPWSTR *ppszwClientDescription)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": CoCreateInstance failed, hr (0x%x)\n", hr);
        return hr;
    }
    ...
    if (fGetWriteLock)
    {
        ...
        if (hr == S_FALSE)
        {
            Log(__FUNCTION__ ": write lock is busy\n", hr);
    ...
    if (hr == S_OK)
    {
        ...
        {
            Log(__FUNCTION__ ": Initialize failed, hr (0x%x)\n", hr);
        }
    }
...
VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinReleaseINetCfg(IN INetCfg *pNetCfg, IN BOOL fHasWriteLock)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": Uninitialize failed, hr (0x%x)\n", hr);
        return hr;
    }
    ...
    if (fHasWriteLock)
    {
        ...
        if (hr != S_OK)
        {
            Log(__FUNCTION__ ": vboxNetCfgWinINetCfgUnlock failed, hr (0x%x)\n", hr);
        }
    }
...
static HRESULT vboxNetCfgWinGetComponentByGuidEnum(IEnumNetCfgComponent *pEnumNcc, IN const GUID * pGuid, OUT INetCfgComponent ** ppNcc)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__": Reset failed, hr (0x%x)\n", hr);
        return hr;
    }
    ...
    while ((hr = pEnumNcc->Next(1, &pNcc, NULL)) == S_OK)
    {
        ...
        if (hr == S_OK)
        {
            if (uComponentStatus == 0)
            {
                ...
                {
                    Log(__FUNCTION__": GetInstanceGuid failed, hr (0x%x)\n", hr);
                }
            }
        }
                                ...
                                OUT INetCfgComponent **ppncc)
{
    ...
    if (hr == S_OK)
    {
        ...
        if (hr == S_FALSE)
        {
            Log(__FUNCTION__": component not found \n");
        }
        else if (hr != S_OK)
        {
            Log(__FUNCTION__": vboxNetCfgWinGetComponentByGuidEnum failed, hr (0x%x)\n", hr);
        }
        pEnumNcc->Release();
    }
    else
    {
        Log(__FUNCTION__": EnumComponents failed, hr (0x%x)\n", hr);
    }
    return hr;
}
...
static HRESULT vboxNetCfgWinQueryInstaller(IN INetCfg *pNetCfg, IN const GUID *pguidClass, INetCfgClassSetup **ppSetup)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": QueryNetCfgClass failed hr (0x%x)\n", hr);
    }
    return hr;
}
        ...
        OUT INetCfgComponent **ppComponent)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": vboxNetCfgWinQueryInstaller failed hr (0x%x)\n", hr);
        return hr;
    }
    ...
    if (hr == S_OK)
    {
        ...
        if (tmpHr != S_OK)
        {
            Log(__FUNCTION__ ": Apply failed, hr (0x%x)\n", tmpHr);
        }
    }
    else
    {
        Log(__FUNCTION__ ": Install failed hr (0x%x)\n", hr);
    }
    pSetup->Release();
    return hr;
}
        ...
        OUT INetCfgComponent **ppComponent)
{
    ...
    for (; i < cInfPaths; i++)
    {
        hr = VBoxNetCfgWinInfInstall(apInfPaths[i]);
        Assert(hr == S_OK);
        if (hr != S_OK)
        {
            Log(__FUNCTION__ ": VBoxNetCfgWinInfInstall failed hr (0x%x)\n", hr);
            break;
        }
    }
    ...
    if (hr == S_OK)
    {
        ...
        if (hr != S_OK)
        {
            Log(__FUNCTION__ ": VBoxNetCfgWinInstallComponent failed hr (0x%x)\n", hr);
        }
    }
    ...
    if (hr != S_OK)
    {
        ...
        {
            VBoxNetCfgWinInfUninstall(apInfPaths[j], 0);
        }
    }
...
VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinUninstallComponent(IN INetCfg *pNetCfg, IN INetCfgComponent *pComponent)
{
    ...
    if (hr != S_OK)
    {
        Log(__FUNCTION__ ": GetClassGuid failed hr (0x%x)\n", hr);
        return hr;
    }
    ...
    {
        Log(__FUNCTION__ ": DeInstall failed hr (0x%x)\n", hr);
    }
    pSetup->Release();
    return hr;
}
        ...
        PVOID pContext)
{
    ...
    if (hr == S_OK)
    {
        ...
        do
        {
            hr = pEnumComponent->Next(1, &pNcc, NULL);
            Assert(hr == S_OK || hr == S_FALSE);
            if (hr == S_OK)
            {
//                ULONG uComponentStatus;
...
//                if (hr == S_OK)
            ...
            else
            {
                ...
                {
                    Log(__FUNCTION__": Next failed, hr (0x%x)\n", hr);
                }
                break;
            }
        } while (!bBreak);
...
static BOOL vboxNetCfgWinRemoveAllNetDevicesOfIdCallback(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext)
{
    ...
    if (SetupDiSetClassInstallParams(hDevInfo,pDev,&rmdParams.ClassInstallHeader,sizeof(rmdParams)))
    {
        if (SetupDiSetSelectedDevice (hDevInfo, pDev))
        {
            if (SetupDiCallClassInstaller(DIF_REMOVE,hDevInfo,pDev))
            {
                ...
                if (SetupDiGetDeviceInstallParams(hDevInfo,pDev,&devParams))
                {
                    ...
                    {
                        hr = S_FALSE;
                        Log(__FUNCTION__": !!!REBOOT REQUIRED!!!\n");
                    }
                }
            }
            else
            {
                winEr = GetLastError();
                Log(__FUNCTION__": SetupDiCallClassInstaller failed winErr(%d)\n", winEr);
                hr = HRESULT_FROM_WIN32(winEr);
            }
        }
        else
        {
            winEr = GetLastError();
            Log(__FUNCTION__": SetupDiSetSelectedDevice failed winErr(%d)\n", winEr);
            hr = HRESULT_FROM_WIN32(winEr);
        }
    }
    else
    {
        winEr = GetLastError();
        Log(__FUNCTION__": SetupDiSetClassInstallParams failed winErr(%d)\n", winEr);
        hr = HRESULT_FROM_WIN32(winEr);
    }
...
VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinEnumNetDevices(LPCWSTR pPnPId, VBOXNETCFGWIN_NETENUM_CALLBACK callback, PVOID pContext)
{
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
        ...
        for (; bEnumCompleted = SetupDiEnumDeviceInfo(hDevInfo, iDev, &Dev); iDev++)
        {
                    ...
                    ))
            {
                ...
                if (winEr != ERROR_INSUFFICIENT_BUFFER)
                {
                    Log(__FUNCTION__": SetupDiGetDeviceRegistryPropertyW (1) failed winErr(%d)\n", winEr);
                    hr = HRESULT_FROM_WIN32(winEr);
                    break;
                }
                ...
                {
                    winEr = GetLastError();
                    Log(__FUNCTION__": SetupDiGetDeviceRegistryPropertyW (2) failed winErr(%d)\n", winEr);
                    hr = HRESULT_FROM_WIN32(winEr);
                    break;
                }
            }
    ...
    {
        DWORD winEr = GetLastError();
        Log(__FUNCTION__": SetupDiGetClassDevsExW failed winErr(%d)\n", winEr);
        hr = HRESULT_FROM_WIN32(winEr);
    }
...
static HRESULT netIfWinFindAdapterClassById(IWbemServices * pSvc, const GUID * pGuid, IWbemClassObject **pAdapterConfig)
{
    ...
    if (length)
    {
        ...
        {
            Log(__FUNCTION__": ExecQuery fail hr (0x%x)\n", hr);
        }
    }
    else
    {
        DWORD winEr = GetLastError();
        Log(__FUNCTION__": StringFromGUID2 fail winEr (%d)\n", winEr);
        hr = HRESULT_FROM_WIN32( winEr );
    }
...
static HRESULT netIfWinCreateIWbemServices(IWbemServices ** ppSvc)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                Log(__FUNCTION__": CoSetProxyBlanket failed hr (0x%x)\n", hr);
            }
        ...
        {
            Log(__FUNCTION__": ConnectServer failed hr (0x%x)\n", hr);
        }
...
static HRESULT netIfWinAdapterConfigPath(IWbemClassObject *pObj, BSTR * pStr)
{
    ...
    {
        DWORD winEr = GetLastError();
        Log(__FUNCTION__": Get failed winEr (%d)\n", winEr);
        Assert(0);
        hr = HRESULT_FROM_WIN32(winEr);
    }
    return hr;
...
VBOXNETCFGWIN_DECL(HRESULT) VBoxNetCfgWinGenHostOnlyNetworkNetworkIp(OUT PULONG pNetIp, OUT PULONG pNetMask)
{
    ...
    {
        Log(__FUNCTION__": GetAdaptersAddresses err (%d)\n", dwRc);
    }
    free(pAddresses);
...
static HRESULT vboxNetCfgWinNetFltUninstall(IN INetCfg *pNc, DWORD InfRmFlags)
{
    ...
    if (hr == S_OK)
    {
        Log("NetFlt Is Installed currently\n");
    ...
    {
        Log("vboxNetCfgWinNetFltUninstall: FindComponent for NetFlt failed, hr (0x%x)\n", hr);
        hr = S_OK;
    }

    VBoxNetCfgWinInfUninstallAll(L"NetService", VBOXNETCFGWIN_NETFLT_ID, InfRmFlags);
    VBoxNetCfgWinInfUninstallAll(L"Net", VBOXNETCFGWIN_NETFLT_MP_ID, InfRmFlags);
...
static BOOL vboxNetCfgWinAdjustHostOnlyNetworkInterfacePriority (IN INetCfg *pNc, IN INetCfgComponent *pNcc, PVOID pContext)
{
    ...
    if ( hr == S_OK )
    {
        ...
        if (hr == S_OK)
        {
            ...
            do
            {
                hr = pEnumNccbp->Next(1, &pNccbp, NULL);
                Assert(hr == S_OK || hr == S_FALSE);
                if (hr == S_OK)
                {
//                   if (pNccbp->IsEnabled() == S_OK)
                    {
                        ...
                        hr = pNccbp->EnumBindingInterfaces(&pEnumNcbi);
                        Assert(hr == S_OK);
                        if (hr == S_OK)
                        {
                            ...
                            do
                            {
                                hr = pEnumNcbi->Next(1, &pNcbi, NULL);
                                Assert(hr == S_OK || hr == S_FALSE);
                                if (hr == S_OK)
                                {
...
//                                            if (uComponentStatus == 0)
                        ...
                        {
                            Log(__FUNCTION__": EnumBindingInterfaces failed, hr (0x%x)\n", hr);
                        }
                    }
        ...
        {
            Log(__FUNCTION__": EnumBindingPaths failed, hr (0x%x)\n", hr);
        }
    ...
    {
        Log(__FUNCTION__": QueryInterface for IID_INetCfgComponentBindings failed, hr (0x%x)\n", hr);
    }
    ...
    do
    {
        ...
        if (pInfPath)
        {
            ...
            if (ok)
            {
                ...
                if (pathLenght < sizeof(DeviceInstallParams.DriverPath)/sizeof(DeviceInstallParams.DriverPath[0]))
                {
                    ...
                    {
                        DWORD winEr = GetLastError();
                        Log("SetupDiSetDeviceInstallParams: SetupDiSetDeviceInstallParams failed, winEr (%d)\n", winEr);
                        Assert(0);
                        break;
                    }
                }
                ...
                {
                    Log("SetupDiSetDeviceInstallParams: inf path is too long\n");
                    Assert(0);
                    break;
                }
            }
            else
            {
                DWORD winEr = GetLastError();
                Assert(0);
                Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: SetupDiGetDeviceInstallParams failed, winEr (%d)\n", winEr);
            }
        ...
        if (ok)
        {
            ...
            else
            {
                DWORD winEr = GetLastError();
                Assert(0);
                Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: SetupInitDefaultQueueCallback failed, winEr (%d)\n", winEr);
            }
        }
        else
        {
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (lppszName)
        {
            *lppszName = ::SysAllocString ((const OLECHAR *) DevName);
            if (!*lppszName)
            {
                Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: SysAllocString failed\n");
                Assert(0);
        ...
        if (pGuid)
        {
            ...
            if (hrc != S_OK)
            {
                Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: CLSIDFromString failed, hrc (0x%x)\n", hrc);
                Assert(0);
            }
        }
        ...
        hr = VBoxNetCfgWinQueryINetCfg(&pNc, TRUE, L"VirtualBox Host-Only Creation",
        ...
        {
            Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: app %S is holding the lock, failed\n", lpszApp);
            CoTaskMemFree(lpszApp);
        }
        else
        {
            Log("VBoxNetCfgWinCreateHostOnlyNetworkInterface: VBoxNetCfgWinQueryINetCfg failed, hr 0x%x\n", hr);
        }
    }
