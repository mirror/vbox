...
SUPR3DECL(int) SUPR3WaitForPowerEvent(RTPOWEREVENT *pEvent)
{
    ...
    *pEvent = RTPOWEREVENT_INVALID;
    ...
    Req.Hdr.u32Cookie = g_u32Cookie;
    Req.Hdr.u32SessionCookie = g_u32SessionCookie;
    Req.Hdr.cbIn = SUP_IOCTL_WAIT_FOR_POWER_EVENT_SIZE_IN;
    Req.Hdr.cbOut = SUP_IOCTL_WAIT_FOR_POWER_EVENT_SIZE_OUT;
    Req.Hdr.fFlags = SUPREQHDR_FLAGS_DEFAULT;
    Req.Hdr.rc = VERR_INTERNAL_ERROR;
    Req.u.Out.enmPowerEvent = RTPOWEREVENT_INVALID;
    int rc = suplibOsIOCtl(&g_supLibData, SUP_IOCTL_WAIT_FOR_POWER_EVENT, &Req, SUP_IOCTL_WAIT_FOR_POWER_EVENT_SIZE);
    if (RT_SUCCESS(rc))
    {
        rc = Req.Hdr.rc;
        if (RT_SUCCESS(rc))
            *pEvent = Req.u.Out.enmPowerEvent;
    }
    return rc;
}
...
SUPR3DECL(int) SUPR3AckPowerEvent(void)
{
    ...
    Req.u32Cookie       = g_u32Cookie;
    Req.u32SessionCookie= g_u32SessionCookie;
    Req.cbIn            = SUP_IOCTL_ACK_POWER_EVENT_SIZE_IN;
    Req.cbOut           = SUP_IOCTL_ACK_POWER_EVENT_SIZE_OUT;
    Req.fFlags          = SUPREQHDR_FLAGS_DEFAULT;
    Req.rc              = VERR_INTERNAL_ERROR;
    int rc = suplibOsIOCtl(&g_supLibData, SUP_IOCTL_ACK_POWER_EVENT, &Req, SUP_IOCTL_ACK_POWER_EVENT_SIZE);
    if (RT_SUCCESS(rc))
        rc = Req.rc;
    return rc;
}
