/* $Id$ */
/** @file
 * InnoTek Portable Runtime - Scheduling, l4env.
 */
...
/** @todo r=bird: Michael, if this is a big stub, we've got sched-generic.cpp for that purpose.
 * Until you've got time to actually implement it please remove this and use sched-generic.cpp!
 */
...
/** Set if we figure we have nice privileges, meaning we can use setpriority
 * to raise the priority. */
bool g_fHaveNicePrivilege = false;
...
static void rtSchedNativeSave(PSAVEDPRIORITY pSave)
{
    ...
    pSave->iPriority = getpriority(PRIO_PROCESS, 0);
    sched_getparam(0, &pSave->SchedParam);
    pSave->iPolicy = sched_getscheduler(0);
    pthread_getschedparam(pthread_self(), &pSave->iPthreadPolicy, &pSave->PthreadSchedParam);
}
#endif
...
static void rtSchedDumpPriority(void)
{
    ...
    Log(("Priority: g_fHaveNicePrivilege=%d g_enmOsPrioSup=%d\n", g_fHaveNicePrivilege, g_enmOsPrioSup));
...
static void *rtSchedNativeProberThread(void *pvUser)
{
    ...
    if (getpriority(PRIO_PROCESS, 0) != iStart)
        g_fHaveNicePrivilege = true;
    else
        g_fHaveNicePrivilege = false;
