...
// VBOX begin
                // this field is sticky, so first reset it for original behavior
                deviceExtension->MouseExtension.CurrentInput.Flags = MOUSE_MOVE_RELATIVE;
                ...
                if (req)
                {
                    ...
                    if (RT_SUCCESS(rc) && RT_SUCCESS(req->header.rc))
                    {
                        if (req->mouseFeatures & VBOXGUEST_MOUSE_HOST_CAN_ABSOLUTE)
                        {
                            ...
                            deviceExtension->MouseExtension.CurrentInput.LastX = req->pointerXPos;
                            deviceExtension->MouseExtension.CurrentInput.LastY = req->pointerYPos;
                        }
                    }
                    else
                    {
