...
#else
static void
util_call_once_data_slow_once(void *context)
{
   struct util_call_once_context_t *once_context = (struct util_call_once_context_t *)context;
   once_context->func(once_context->data);
}
...
util_call_once_data_slow(once_flag *once, util_call_once_data_func func, const void *data)
{
   ...
   once_context.data = data;
   once_context.func = func;
   call_once_data(once, util_call_once_data_slow_once, &once_context);
}
#endif
