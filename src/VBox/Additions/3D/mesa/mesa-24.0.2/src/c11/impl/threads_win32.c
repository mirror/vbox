...
#ifdef IPRT_NO_CRT
/*
 * 'impl_current_thread' (and 'impl_thrd_param::thrd') are used for keeping a thread handle.
 * This is not needed with VBox, because it uses 'RTThreadSelfAutoAdopt' to get the current thread handle.
 */
#endif
...
#ifdef IPRT_NO_CRT
struct impl_call_once_param_data { void (*func)(void *data); void *data; };
static BOOL CALLBACK impl_call_once_callback_data(PINIT_ONCE InitOnce, PVOID Parameter, PVOID *Context)
{
    struct impl_call_once_param_data *param = (struct impl_call_once_param_data*)Parameter;
    (param->func)(param->data);
    ((void)InitOnce); ((void)Context);  // suppress warning
    return true;
}
#endif
...
#ifdef IPRT_NO_CRT
void
call_once_data(once_flag *flag, void (*func)(void *data), void *data)
{
    assert(flag && func);
    struct impl_call_once_param_data param;
    param.func = func;
    param.data = data;
    InitOnceExecuteOnce((PINIT_ONCE)flag, impl_call_once_callback_data, (PVOID)&param, NULL);
}
#endif
