...
#endif
{
    ...
    if (NT_SUCCESS(rc))
        rc = vbgdNtScanPCIResourceList(pStack->Parameters.StartDevice.AllocatedResourcesTranslated,
                                             pDevExt);
#endif
    if (NT_SUCCESS(rc))
    {
    ...
    if (NT_SUCCESS(rc))
    {
        ...
        IoInitializeDpcRequest(pDevExt->win.s.pDeviceObject, vbgdNtDpcHandler);
#ifdef TARGET_NT4
