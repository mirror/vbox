  ...
  )
{
  ...
  if (PlatformInfoHob->LowMemory < McfgBase)
    PlatformAddIoMemoryRangeHob (PlatformInfoHob->LowMemory, McfgBase);
  PlatformAddIoMemoryRangeHob (McfgBase + McfgSize, 0xFC000000);
  ...
  )
{
  ...
  GetVmVariable(EFI_INFO_INDEX_CPU_COUNT_MAX, (CHAR8 *)&MaxCpuCount, sizeof(MaxCpuCount));
#endif
