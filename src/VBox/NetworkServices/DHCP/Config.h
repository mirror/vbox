 ...
 */
typedef std::map<Client, Lease*> MapClient2Lease;
...
public:
    ...
    static Lease *generateLease(const Client& client);
protected:
    /** returns already allocated lease or NULL */
    Lease *getLease(const Client& client) const;
    /** generates new lease by client query and configuration rules */
    Lease *createLease(const Client& client, const BaseConfigEntity *cfg) const;
private:
    LeaseGenerator(){}
    virtual ~LeaseGenerator(){}
    std::map<Client, Lease*> m_leases;
};
