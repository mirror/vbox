...
int Appliance::writeS3(TaskWriteOVF *pTask)
{
    ...
    char *pszTmpDir;
    RTStrAPrintf(&pszTmpDir, "%s/vbox-ovf-XXXXXX", szOSTmpDir);
    ...
    for (list< pair<Utf8Str, ULONG> >::const_iterator it1 = filesList.begin(); it1 != filesList.end(); ++it1)
    {
        ...
        if(RT_FAILURE(vrc))
            rc = setError(VBOX_E_FILE_ERROR,
                          tr("Cannot delete file '%s'"), s.first.c_str());
    }
    ...
    if (RTPathExists(pszTmpDir))
    {
        ...
        if(RT_FAILURE(vrc))
            rc = setError(VBOX_E_FILE_ERROR,
                          tr("Cannot delete temporary directory '%s'"), pszTmpDir);
    }
    if (pszTmpDir)
        RTStrFree(pszTmpDir);
