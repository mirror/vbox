 ...
 */
static const uint8_t g_aDarwinToSet1[] =
{
...
/**
 * Converts a key event to a set 1 scan code.
 *
 * @returns set 1 scan code.
 * @param   Event               The event.
 * @param   pfCurKeyModifiers   Where the current key modifier mask is stored. (in/out)
 */
unsigned DarwinEventToSet1Scancode(EventRef Event, UInt32 *pfCurKeyModifiers)
{
    UInt32 EventClass = GetEventClass(Event);
    if (RT_UNLIKELY(EventClass != kEventClassKeyboard))
        return 0;
    ...
    switch (EventKind)
    {
        ...
        {
            UInt32 uKeyCode;
            GetEventParameter(Event, kEventParamKeyCode, typeUInt32, NULL, sizeof(uKeyCode), NULL, &uKeyCode);
            unsigned uScanCode = DarwinKeycodeToSet1Scancode(uKeyCode);
            if (uScanCode == 0)
            {
                /* modifiers and other hacks? */
                /** @todo fallback... */
            }
#ifdef DEBUG_PRINTF
            RTPrintf("dbg: EventKind=%d: %#x -> %#x\n", EventKind, uKeyCode, uScanCode);
#endif
            return uScanCode;
        }
        ...
        {
            /* get and update the key modifiers. */
            const UInt32 fOldKeyModifiers = *pfCurKeyModifiers;
            UInt32 fKeyModifiers = ~0;
            if (GetEventParameter(Event, kEventParamKeyModifiers, typeUInt32, NULL, sizeof(fKeyModifiers), NULL, &fKeyModifiers))
            {
#ifdef DEBUG_PRINTF
                RTPrintf("dbg: EventKind=%d: 1\n");
#endif
                return 0;
            }

            /* calc changes and see if they apply to us. */
            UInt32 fChanged = fKeyModifiers ^ fOldKeyModifiers;
            fChanged &= shiftKey | rightShiftKey | controlKey | rightControlKey | optionKey | rightOptionKey | cmdKey;
            if (!fChanged)
            {
#ifdef DEBUG_PRINTF
                RTPrintf("dbg: EventKind=%d: 2\n");
#endif
                *pfCurKeyModifiers = fKeyModifiers;
                return 0;
            }

            /* see what changed - it better not combine changes! */
            unsigned uScanCode = DarwinModifierMaskToSet1Scancode(fChanged);
            if (uScanCode && (fChanged & fKeyModifiers))
                uScanCode |= 0x8000; /* 0x8000 == key down hack. */ /** @todo define. */
#ifdef DEBUG_PRINTF
            RTPrintf("dbg: EventKind=%d: %#x ^ %#x = %#x -> %#x\n", EventKind,
                     fOldKeyModifiers, fKeyModifiers, fChanged, uScanCode);
#endif
            if (uScanCode != ~0U)
                *pfCurKeyModifiers = fKeyModifiers;
            return uScanCode;
