...
static int patmReinit(PVM pVM)
{
    ...
    rc = CPUMR3QueryGuestCtxGCPtr(pVM, &pVM->patm.s.pCPUMCtxGC);
    AssertRCReturn(rc, rc);
        ...
        if (pRec->opcode == OP_CALL)
        {
            ...
            if (PATMQueryFunctionPatch(pVM, pRec->pTargetGC) == 0)
            {
                ...
                if (VBOX_FAILURE(rc))
                {
                    ...
                    Log(("Failed to install function replacement patch (at %x) for reason %Vrc\n", pOrgInstrGC, rc));
...
static int patmAnalyseBlockCallback(PVM pVM, DISCPUSTATE *pCpu, GCPTRTYPE(uint8_t *) pInstrGC, GCPTRTYPE(uint8_t *) pCurInstrGC, void *pUserData)
{
    ...
    else
    {
        ...
        {
            Assert(pCpu->param1.size <= 4 || pCpu->param1.size == 8);
            if (    pCpu->param1.size == 8 /* far call/jmp */
