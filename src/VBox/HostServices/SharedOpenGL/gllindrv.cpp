...
int vboxglEnableOpenGL(PVBOXOGLCTX pClient)
{
    ...
    if (!pClient->glxContext) 
    {
        ...
        pClient->enable.visinfo = pClient->glxGetVisualFromFBConfig(pClient->enable.dpy, pClient->enable.fbConfig[0]);
        /* Create Window */
        attr.background_pixel = 0;
        attr.border_pixel = 0;
        attr.colormap = XCreateColormap(pClient->dpy, RootWindow(pClient->enable.dpy, screen_num), pClient->enable.visinfo->visual, AllocNone);
