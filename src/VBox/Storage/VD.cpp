...
typedef struct VDIOCTX
{
    ...
    union
    {
        ...
        struct
        {
            ...
            /** S/G buffer */
            RTSGBUF              SgBuf;
            /** Flag whether all reads from the image chain returned VERR_VD_BLOCK_FREE
             * so far. */
            bool                 fAllFree;
...
static int vdReadHelperAsync(PVDIOCTX pIoCtx)
{
    ...
    return (!(pIoCtx->fFlags & VDIOCTX_FLAGS_ZERO_FREE_BLOCKS) && pIoCtx->Req.Io.fAllFree)
           ? VERR_VD_BLOCK_FREE
           : rc;
}
                                  ...
                                  size_t cbCompleted)
{
    ...
    if (ASMAtomicCmpXchgBool(&pDisk->fLocked, true, false))
    {
        /* Immediately drop the lock again, it will take of processing the list. */
        vdDiskUnlock(pDisk, NULL);
    }
}
