 ...
 * when reading from such File instances.
 *
 * @todo We should duplicate file handles (once this API is in IPRT) passed to
 *       constructors to provide thread-safety.
 */
...
public:
     ...
     * @param aFileName File name (for reference, may be NULL).
     */
    File (Mode aMode, RTFILE aHandle, const char *aFileName);
#endif
